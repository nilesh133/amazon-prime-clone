                    ...
                    <div>
                        ...
                        <section className='relative bg-gradient'>
                            ...
                            <div className='absolute left-[5%] md:left-14 top-12 w-[90%]'>
                                <h3 className='text-[30px] md:text-[35px] font-bold text-white'>{result.name}</h3>
                                <p className='text-md w-[100%] md:text-lg md:w-[75%] font-semibold text-white mt-3'>{result.overview}</p>
                                <div className='mt-5 flex items-center'>
                                    <div onClick={() => setShowTrailer(true)} className="text-md md:text-lg bg-[#0f79af] text-white font-spartan px-4 pt-2 pb-1 rounded-sm font-medium
                        tracking-wider mr-4 border-2 border-white transition-all hover:bg-[#379cce] hover:text-white cursor-pointer">Watch Trailer</div>
                                    <div className='flex'>
                                        <span className='text-xl p-3 md:p-4 rounded-full bg-[#425265] text-white ml-2 cursor-pointer hover:bg-[#384657]'><ImPlay3 /></span>
                                        <span className='text-xl p-3 md:p-4 rounded-full bg-[#425265] text-white ml-2 cursor-pointer hover:bg-[#384657]'><BsPlusLg /></span>
                                        <span className='text-xl p-3 md:p-4 rounded-full bg-[#425265] text-white ml-2 cursor-pointer hover:bg-[#384657]'><FiDownload /></span>
                                    </div>
                                </div>
                                <div className='mt-8'>
                                    <table>
                                        ...
                                        <tr>
                                            <td className='text-md md:text-lg text-[#8197A4] font-bold'>Total Seasons:</td>
                                            <td className='text-md md:text-lg text-[#79B8F3] font-bold pl-2'>{result.number_of_seasons}</td>
                                        </tr>
                                        <tr>
                                            <td className='text-md md:text-lg text-[#8197A4] font-bold'>Total Episodes:</td>
                                            <td className='text-md md:text-lg text-[#79B8F3] font-bold pl-2'>{result.number_of_episodes}</td>
                                        </tr>
                                        <tr>
                            ...
                            <div className={`absolute top-3 inset-x-[7%] md:inset-x-[13%] overflow-hidden transition duration-500 ${showTrailer ? "opacity-100 z-50" : "opacity-0 z-[-10]"}`}>
                                <div className='flex justify-between items-center bg-black text-white p-4' >
                                    ...
                                    <span className='hover:text-gray-300 transition-all cursor-pointer' onClick={() => setShowTrailer(false)}><ImCancelCircle className='text-[25px]' /></span>
                                </div>
...
export async function getServerSideProps(ctx) {
    ...
    const movieDetails = await fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=${process.env.API_KEY}&append_to_response=videos`)
        .then((res) => res.json())
    return {
        props: {
