...
const Show = ({ result }) => {
    ...
    const title = `${result.name} (${result.first_air_date.split("-")[0]})`;
    return (
        <div onClick={() => router.push(`/shows/${result.id}`)} className='relative w-64 h-40 md:w-96 md:h-56 group flex flex-col cursor-pointer rounded-lg transition-all transform hover:border-4 hover:border-[#0f79af]' key={result.id}>
